...
import Server3.MontrealServerImpl;
...
public class MontrealServerImpl1 extends ClientServerInterfacePOA implements Runnable  {
	...
	public static MontrealServerImpl1 add;

	public MontrealServerImpl1()  {
		super();
	}
	...
	{
		String Path1 = "LogServer/" + "MontrealServerImpl1Log.txt";
		try {
	...
	public static void main(String args[]) {
		try {
			...
			Thread threadSocket = new Thread(new MontrealServerImpl1());
			threadSocket.start();
			System.out.println("######### Montreal One Server is up and running.########");
			...
			try
			{
				socket = new DatagramSocket(6170);
	...
	public synchronized  String addEvent(String eventType, String eventId,int bookingCapactiy)  {
		...
		System.out.println("########## EventType : " + eventType + " \t with Event ID : " + eventId + "  was added  with capacity "
	...
	public synchronized String bookEvent(String customerId, String eventType,String eventId)  {
		...
		if (city.equalsIgnoreCase("mtl")) {
			...
			System.out.println(" ####### Booking an ottawa event from montreal server through udp server communication ######### ");
			...
			if(count < 3) {
				try {
					...
					DatagramPacket request1 = new DatagramPacket(message1, message1.length, address, 7181);
...
;			}
			...
			System.out.println(" ####### Booking an toronto event from montreal server through udp server communication ######### ");
			...
			try {
				...
				InetAddress address = InetAddress.getByName("localhost");
				System.out.print("count is :"+count);
				DatagramPacket request2 = new DatagramPacket(message2, message2.length, address, 7182);
	...
	public   String getBookingSchedule(String customerId)  {
		...
		Runnable task1 = () -> {
			String s1 = senderOne(customerId,null,null,"getBooking",7181);
			if(!s1.equals(" "))
			{
			st.add(s1);
			}
		...
		Runnable task2 = () -> {
			String  s2 =  senderTwo(customerId,null,null,"getBooking",7182);
			if(!s2.equals(" "))
			{
			st.add(s2);
			}
	...
	public  synchronized String cancelEvents(String cutomerId, String eventType,String eventId)  {
		...
		else if(city.equalsIgnoreCase("tor"))
		{
			...
			try {
				...
				DatagramPacket request2 = new DatagramPacket(message1, message1.length, address, 7182);
				socket2.send(request2);
				System.out.println("Request message sent from the client is : " + new String(request2.getData()));
	...
	public  String listEventAvailability(String eventType)  {
		...
		List<String> st = new ArrayList<String>();
		Runnable task1 = () -> {
			String s1 = senderOne(null,eventType,null,"listEventAvailability",7181);
			if(!s1.equals(" "))
			{
			st.add(s1);
			}
		...
		Runnable task2 = () -> {
			String  s2 =  senderTwo(null,eventType,null,"listEventAvailability",7182);
			if(!s2.equals(" "))
			{
			st.add(s2);
			}
	...
	public void run() {
		...
		DatagramSocket socket = null;
		MontrealServerImpl1 add = new MontrealServerImpl1();
		try {
			socket = new DatagramSocket(7180);
			while (true) {
