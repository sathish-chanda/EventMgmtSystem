...
import ClientServerInterfaceApp.ClientServerInterface;
...
public class MontrealServerImpl extends ClientServerInterfacePOA implements Runnable {
	// private static Logger LOGGER = LogManager.getLogger("mont");

	private static final long serialVersionUID = 1L;
	public static Map<String, Map<String, EventDetails>> mtlMap = new HashMap<String, Map<String, EventDetails>>();
	...
	private org.omg.CORBA.ORB orbMontreal;
	
	public static MontrealServerImpl add; 
   
	
    public static MontrealServerImpl getInstance() {
        if (add == null) {
        	add = new MontrealServerImpl(); }
        return add;
    }

	protected MontrealServerImpl() {
		super();
	}
	...
	{
		String Path1 = "LogServer/" + "MontrealServerImplLog.txt";
		try {
	...
	public static void main(String args[]) {
		try {
			...
			add = new MontrealServerImpl();
			add.addEvent("C", "MTLM000000", 1);
			add.addEvent("S", "MTLM000000", 1);
			add.addEvent("T", "MTLM000000", 1);
			
			
			Thread threadSocket = new Thread(new MontrealServerImpl());
			threadSocket.start();
			...
			System.out.println("###### Montreal cControl started #######");
			try
			{
	...
	public synchronized String addEvent(String eventType, String eventId, int bookingCapactiy) {
		...
		System.out.println("########## Montreal Server :: Add Event ############");
		String returnMessage = "";
		add.logWritter("########## Montreal Server :: Add Event ############");
		System.out.println("event id is :"+eventId);
		if (eventId.substring(0, 3).equalsIgnoreCase("mtl")) {
			if (!mtlMap.isEmpty()) {

				if (mtlMap.containsKey(eventType)) {
					submap = mtlMap.get(eventType);

					if (!submap.containsKey(eventId)) {
						submap.put(eventId, e);
						mtlMap.put(eventType, submap);
			...
			returnMessage = "###### Montreal manager can not add an event to another location ######";
			System.out.println("###### Montreal manager can not add an event to another location ######");
		}

		System.out.println("########## EventType : " + eventType + " \t with Event ID : " + eventId
				+ "  was added  with capacity " + bookingCapactiy + " from montreal server ##########");

		add.logWritter("######### EventType : " + eventType + " \t with Event ID : " +
		 eventId + " was added with capacity "
		 + bookingCapactiy + " from montreal server ###########");
	...
	public synchronized String deleteEvent(String eventType, String eventId) {
		...
		System.out.println("########## Montreal Server :: Remove Event ############");
		add.logWritter("########## Montreal Server :: Remove Event ############");
	...
	public void printHashMap() {
		...
		for (Map.Entry<String, Map<String, EventDetails>> entry : mtlMap.entrySet()) {
	...
	public Map<String, Map<String, EventDetails>> print() {
	...
	public synchronized String bookEvent(String customerId, String eventType, String eventId) {
		...
		System.out.println("########## Montreal Server :: Book Event ############");
		
		add.logWritter("########## Montreal Server :: Book Event ############");

		int count = 0;
		String returnMessage = "";
		
		System.out.println("montreal server :"+eventId);
		...
		if (city.equalsIgnoreCase("mtl")) {
			...
			if (mtlMap.get(eventType) != null) {
				...
				if (submap.containsKey(eventId)) {
					...
					if (e.seatsLeft > 0 && e.seatsFilled <= e.bookingCapacity) {
						...
						submap.put(eventId, e);
						mtlMap.put(eventType, submap);
						System.out.println("########## Customer with customer ID  :" + customerId
								+ " booked event type of  : " + eventType + " with event ID : ########### " + eventId);
			...
			System.out.println(
					" ####### Booking an ottawa event from montreal server through udp server communication ######### ");
			add.logWritter(" ####### Booking an ottawa event from montreal server through udp server communication ######### ");
			...
			if (count < 3) {
				try {
					...
					DatagramPacket request1 = new DatagramPacket(message1, message1.length, address, 7081);
					socket1.send(request1);
					System.out.println("########## Request message sent from the client is  ############ : "
							+ new String(request1.getData()));
					...
					System.out.println("######### Request message received by the client is ############ : "
			...
			System.out.println(
					" ####### Booking an toronto event from montreal server through udp server communication ######### ");
			add.logWritter(" ####### Booking an toronto event from montreal server through udp server communication ######### ");
			...
			if (count < 3) {
				...
				try {
					...
					InetAddress address = InetAddress.getByName("localhost");
					System.out.print("count is :" + count);
					DatagramPacket request2 = new DatagramPacket(message2, message2.length, address, 7082);
					socket2.send(request2);
					System.out.println("######### Request message sent from the client is : ########## :  "
							+ new String(request2.getData()));
				...
				returnMessage = "You can't register this event because In a month more than 3 events outside the city is not allowed";
				System.out.println("##########  Outside city event booking threadshold breached ##########");
				add.logWritter("########## Outside city event booking threadshold breached ##########");
				returnValue = false;
			}

		}

		System.out.println("######### End of book event #############");
		printHashMap();
		return returnMessage;
	...
	public String getBookingSchedule(String customerId) {
		...
		Runnable task1 = () -> {
			String s1 = senderOne(customerId, null, null, "getBooking", 7081);
			if(!s1.equals(" "))
			{
				st.add(s1);
			}
		...
		Runnable task2 = () -> {
			String s2 = senderTwo(customerId, null, null, "getBooking", 7082);
			if(!s2.equals(" "))
			{
				st.add(s2);
			}
	...
	public synchronized String getCurrentBookingSchedule(String customerId) {
		...
		System.out.println("Checking if the customer id ");
	...
	public synchronized String cancelEvents(String cutomerId, String eventType, String eventId) {
		...
		if (city.equalsIgnoreCase("mtl")) {
			...
			if (submap != null) {
				for (Map.Entry<String, EventDetails> e : submap.entrySet()) {
					...
					if (customerRegistered.contains(cutomerId)) {
						...
						System.out.println("Customer with customer ID : " + cutomerId + "cancelled the event with ID : "
								+ eventId);
			...
			int count = 0;
			if (cusMapCount.containsKey(cutomerId)) {
				count = cusMapCount.get(cutomerId);
			}
			...
			try {
				...
				DatagramPacket request2 = new DatagramPacket(message1, message1.length, address, 7082);
				socket2.send(request2);
	...
	public String listEventAvailability(String eventType) {
		...
		System.out.println("########## Montreal Server :: List Event ############");
		add.logWritter("########## Montreal Server :: List Event ############");
		...
		List<String> st = new ArrayList<String>();
		Runnable task1 = () -> {
			String s1 = senderOne(null, eventType, null, "listEventAvailability", 7081);
			if(!s1.equals(" "))
			{
				st.add(s1);
			}
		...
		Runnable task2 = () -> {
			String s2 = senderTwo(null, eventType, null, "listEventAvailability", 7082);
			if(!s2.equals(" "))
			{
			st.add(s2);
			}
	...
	public synchronized String getCurrentServerEventAvailability(String eventType) {
		System.out.println("########## Montreal Server :: Current List Event ############");
		add.logWritter("########## Montreal Server :: Current List Event ############");
		...
		if (mtlMap.get(eventType) != null) {
			...
			System.out.println("##### EventType : " + eventType + " : is not avaliable in the current server #####");
		}

		add.logWritter("########## Montreal Server :: Current List Event End ############");

		return s;

	}

	public synchronized String senderOne(String customerId, String eventType, String eventId, String requestType,
			int port) {
		String result = "";
		String returnString = "";

		System.out.println("########## Montreal Server :: senderOne ############");
		add.logWritter("########## Montreal Server :: senderOne ############");
		...
		try {
			if (requestType.equalsIgnoreCase("listEventAvailability")) {
				...
				System.out.println(
						"######### Reuqest Type for server to server communication is ######### :" + requestType);
		...
		System.out.println("########## Montreal Server :: senderOne End ############");
		add.logWritter("########## Montreal Server :: senderOne End ############");

		return returnString;

	}

	public synchronized String senderTwo(String customerId, String eventType, String eventId, String requestType,
			int port) {

		String result = "";
		String returnString = "";

		System.out.println("########## Montreal Server :: senderTwo  ############");
		add.logWritter("########## Montreal Server :: senderTwo ############");
		...
		System.out.println("########## Montreal Server :: senderTwo End ############");
		add.logWritter("########## Montreal Server :: senderTwo End ############");
...
public void run() {
		...
		DatagramSocket socket = null;
		MontrealServerImpl add = new MontrealServerImpl();
		try {
			socket = new DatagramSocket(7080);
			while (true) {
				...
				else if(requestType.equalsIgnoreCase("getBooking"))
				{
					...
					//result  = add.getBookingSchedule(splitMsg[0]);
