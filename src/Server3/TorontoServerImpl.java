...
public class TorontoServerImpl extends ClientServerInterfacePOA implements Runnable {
	...
	public static TorontoServerImpl add;
	...
	{
		String Path1 = "LogServer/" + "TorontoServerImplLog.txt";
		try {
	...
	public static void main(String args[]) {
		...
		try {
						...
						try
						{
							socket = new DatagramSocket(6072);
							while(true)
							{
		...
		if (city.equalsIgnoreCase("tor")) {
			...
			if (count < 3) {
				...
				try {
					...
					DatagramPacket request2 = new DatagramPacket(message1, message1.length, address, 7081);
	...
	public   String getBookingSchedule(String customerId)  {
		...
		Runnable task2 = () -> {
			String  s2 =  senderTwo(customerId,null,null,"getBooking",7081);
			if(!s2.equals(" "))
			{
			st.add(s2);
			}
		...
		if (customerLis.size() == 0) {
			...
			for (CustomerDetails c : customerLis) {
				if (c.getCustomerID().equals(customerId)) {
					...
					//returnStr = returnStr +  "#" + c.getEventId()  + c.getEventType() +",";
...
public synchronized String cancelEvents(String cutomerId,String eventType,String eventId)  {
	...
	public  String listEventAvailability(String eventType)  {
		...
		Runnable task2 = () -> {
			String  s2 =  senderTwo(null,eventType,null,"listEventAvailability",7081);
			if(!s2.equals(" "))
			{
			st.add(s2);
			}
	...
	public void run() {
		DatagramSocket socket = null;
		TorontoServerImpl add = new TorontoServerImpl();
		try {

			socket = new DatagramSocket(7082);
