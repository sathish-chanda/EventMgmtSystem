...
//import com.concordia.entity.CustomerDetails;
//import com.concordia.entity.EventDetails;
...
public class OttawaServerImpl extends ClientServerInterfacePOA implements Runnable  {
	//private static Logger LOGGER = LogManager.getLogger("ottawa");

	private static final long serialVersionUID = 1L;
	public static Map<String, Map<String, EventDetails>> otwMap = new HashMap<String, Map<String, EventDetails>>();
    ...
    public static OttawaServerImpl add; 
   
	
    public static OttawaServerImpl getInstance() {
        if (add == null) {
        	add = new OttawaServerImpl(); }
        return add;
    }
	...
	private org.omg.CORBA.ORB orbOttawa;
	...
	public  void shutdown() {
		orbOttawa.shutdown(false);
	}

	public void logWritter(String log)
	{
		String Path1 = "LogServer/" + "OttawaServerImplLog.txt";
		try {
	...
	public static void main(String args[]) {
		...
		try {
			        ...
			        add = new OttawaServerImpl(); 
			        add.addEvent("C", "OTWM000000", 1);
			        add.addEvent("S", "OTWM000000", 1);
			        add.addEvent("T", "OTWM000000", 1);
					
					Thread threadSocket = new Thread(new OttawaServerImpl());
					threadSocket.start();
					System.out.println("######### Ottawa Server is up and running.########");
					DatagramSocket socket = null;
					System.out.println("###### Ottawa cControl started #######");
					
					
					try
					{
						socket = new DatagramSocket(6071);
						while(true)
						{
					...
					System.out.println("Ottawa Server Start up failed: " + e);
					e.printStackTrace();
				}
	...
	public synchronized  String addEvent(String eventType, String eventId,int bookingCapactiy)  {
		...
		System.out.println("########## Ottawa Server :: Add Event ############");
		String returnMessage = "";
		add.logWritter("########## Ottawa Server :: Add Event ############");
		if(eventId.substring(0, 3).equalsIgnoreCase("otw"))
		{
		if (!otwMap.isEmpty()) {

			if (otwMap.containsKey(eventType)) {
				submap = otwMap.get(eventType);

				if (!submap.containsKey(eventId)) {
					submap.put(eventId, e);
					otwMap.put(eventType, submap);
		...
		{
			returnMessage = "###### Ottawa manager can not add an event to another location ######";
			System.out.println("###### Ottawa manager can not add an event to another location ######");
		}

		System.out.println("########## EventType : " + eventType + "\t with Event ID : " + eventId + " was added with capacity "
				+ bookingCapactiy + " from ottawa server  ########## ");
		
		
		add.logWritter("####### EventType : " + eventType + "\t with Event ID : " + eventId + "  was added  with capacity "	+ bookingCapactiy + " from ottawa server #########");
	...
	public synchronized String deleteEvent(String eventType,String eventId)  {
		...
		System.out.println("########## Ottawa Server :: Remove Event ############");
		add.logWritter("########## Ottawa Server :: Remove Event ############");

	
		if (otwMap.get(eventType) != null) {
	...
	public  void printHashMap()  {
		...
		for (Map.Entry<String, Map<String, EventDetails>> entry : otwMap.entrySet()) {
	...
	public synchronized String bookEvent(String customerId, String eventType,String eventId)  {
		...
		System.out.println("########## Ottawa Server :: Book Event ############");
		add.logWritter("########## Ottawa Server :: Book Event ############");
		...
		System.out.println("request message is :"+requestMessage);	
		...
		if (city.equalsIgnoreCase("otw")) {
			...
			if (otwMap.get(eventType) != null) {
				...
				if (submap.containsKey(eventId)) {
					...
					if (e.seatsLeft > 0 && e.seatsFilled <= e.bookingCapacity) {
						...
						add.logWritter("########## Customer with customer ID  :" + customerId + " booked event type of  : "	+ eventType + " with event ID : ########### " + eventId);
			...
			if(count < 3) {
				try {
					...
					System.out.println(" ####### Booking an montreal event from ottawa server through udp server communication ######### ");
					add.logWritter("    ####### Booking an montreal event from ottawa server through udp server communication ######### ");
					System.out.print("count is :"+count);
					socket1 = new DatagramSocket();

					InetAddress address = InetAddress.getByName("localhost");

					DatagramPacket request1 = new DatagramPacket(message1, message1.length, address, 7080);
			...
			System.out.println(" ####### Booking an toronto event from ottawa server through udp server communication ######### ");
			add.logWritter("    ####### Booking an toronto event from ottawa server through udp server communication ######### ");
	...
	public  String getBookingSchedule(String customerId)  {
		...
		Runnable task1 = () -> {
			String s1 = senderOne(customerId,null,null,"getBooking",7080);
			if(!s1.equals(" "))
			{
			st.add(s1);
			}
		...
		if (customerLis.size() == 0) {
			...
			for (CustomerDetails c : customerLis) {
				if (c.getCustomerID().equals(customerId)) {
					lis.add(c);
					//returnStr = returnStr + c.getCustomerID() + " " + c.getEventType() + " " + c.getEventId() + ",";
	...
	public  String listEventAvailability(String eventType)  {
		...
		System.out.println("########## Ottawa Server :: List Event ############");
		add.logWritter("########## Ottawa Server :: List Event ############");
		...
		List<String> st = new ArrayList<String>();
		Runnable task1 = () -> {
			String s1 = senderOne(null,eventType,null,"listEventAvailability",7080);
			if(!s1.equals(" "))
			{
			st.add(s1);
			}
	...
	public synchronized String senderTwo(String customerId,String eventType,String eventId,String requestType,int port)
	{
		...
		System.out.println("########## Ottawa Server :: senderTwo  ############");
		add.logWritter("########## Ottawa Server :: senderTwo  ############");
	...
	public void run() {
		DatagramSocket socket = null;
		OttawaServerImpl add = new OttawaServerImpl();
		try {

			socket = new DatagramSocket(7081);
