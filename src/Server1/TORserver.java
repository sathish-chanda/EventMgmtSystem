...
public class TORserver  {
	...
	public static int TORport = 2347;
	
	public static TORserver server;
		...
		if(eventsdata.get(eventype).containsKey(eventid))
		{
		   ...
		   DatagramSocket socket2 = null;
			...
			try {
				...
				socket2 = new DatagramSocket();
				DatagramPacket tor = new DatagramPacket(send,send.length,address,MTLserver.MTLport);
				socket2.send(tor);
				byte[] get2 = new byte[2000];
				DatagramPacket receive2 = new DatagramPacket(get2,get2.length);
				socket2.receive(receive2);
